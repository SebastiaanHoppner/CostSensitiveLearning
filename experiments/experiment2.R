...
# save results as ----------------------------
save_results_as <- paste0("experiment2_", Sys.Date(), "_1.RData")
...
creditcard$transactionID          <- as.character(creditcard$transactionID)
creditcard$creditcardID           <- as.character(creditcard$creditcardID)
creditcard$merchantID             <- as.character(creditcard$merchantID)

creditcard$datetime               <- as.POSIXct(creditcard$datetime)

creditcard <- creditcard %>% select(isFraud,
...
creditcard$freq_past_merc      <- log(1e-5 + creditcard$freq_past_merc)
creditcard$merchant_rel_degree <- log(1e-5 + creditcard$merchant_rel_degree)
#creditcard$freq_merchantID     <- log(1e-5 + creditcard$freq_merchantID)
#creditcard$freq_category       <- log(1e-5 + creditcard$freq_category)
#creditcard$freq_country_cat    <- log(1e-5 + creditcard$freq_country_cat)
#creditcard$freq_time_CI_flag   <- log(1e-5 + creditcard$freq_time_CI_flag)
#creditcard$freq_geq_amount     <- log(1e-5 + creditcard$freq_geq_amount)

#creditcard[, 9:15] <- scale(creditcard[, 9:15])
creditcard[, 9:10] <- scale(creditcard[, 9:10])

# summary(creditcard)
# apply(creditcard[, 8:16], 2, mean)
# apply(creditcard[, 8:16], 2, sd)
# boxplot(creditcard[, 8:16])
# cor(creditcard[, 8:16])
# VIFs <- diag(solve(cor(creditcard[, 8:16])))
# sort(VIFs)
# mean(VIFs)
# max(VIFs)
# e <- eigen(cor(creditcard[, 8:16]))$values
# sqrt(max(e)/min(e))
...
for (k in 1:nruns) {
  for (j in 1:nfolds) {
    ...
    cslogitL1 <- cslogit(formula     = isFraud ~ .,
                         data        = train,
    ...
    set.seed(seed)
    lassoCV <- cv.glmnet(x            = model.matrix(isFraud ~ ., data = train)[, -1],
                         y            = train$isFraud,
                         family       = "binomial",
                         alpha        = 1,
                         intercept    = TRUE,
    ...
    set.seed(seed)
    ridgeCV <- cv.glmnet(x            = model.matrix(isFraud ~ ., data = train)[, -1],
                         y            = train$isFraud,
                         family       = "binomial",
                         alpha        = 0,
                         intercept    = TRUE,
    ...
    set.seed(seed)
    elnetCV <- cv.glmnet(x            = model.matrix(isFraud ~ ., data = train)[, -1],
                         y            = train$isFraud,
                         family       = "binomial",
                         alpha        = 0.5,
                         intercept    = TRUE,
    ...
    linda <- lda(formula = isFraud ~ ., data = train, method = "moment")
    
    
    
    # Performance of the models -------------------------------------------------------------------
    validGlmnet <- model.matrix(isFraud ~ ., data = valid)[, -1]
    ...
    results_cslogitL1 <- rbind.data.frame(results_cslogitL1,
                                          performanceMeasures(scores_cslogitL1, thresholds,
                                                              valid$isFraud, cost_matrix_valid))
