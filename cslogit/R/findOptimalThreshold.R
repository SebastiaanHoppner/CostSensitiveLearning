findOptimalThreshold <- function (scores, true_classes, cost_matrix, thresholds, show = TRUE) {
  # original call
  call <- match.call()
  ...
  if (show) {
    plot(thresholds, savings, type = "l", lwd = 2)
    points(optimal_threshold, max(savings), pch = 16, col = "red")
  }
